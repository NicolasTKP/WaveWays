...
def scrape_weather_data(url):
    """
    Scrapes weather data from the given URL.
    """
    try:
            ...
            for row in table.find('tbody').find_all('tr'):
                ...
                for j, header in enumerate(headers):
                    if j < len(row_data):
                        ...
                        row_dict[header] = None # Handle missing data for a header
                
                # Further parse the 'Forecast' column if it exists
...
def get_optimal_path_route(ports_gdf, vessel_data, num_sample_ports=4, random_state=50):
    """
    ...
    if not reachable_edges:
        ...
        if not G_reachable.nodes:
            ...
            return path, G, selected_ports # Return G and selected_ports for later use

# --- From test2.py (functions) ---
...
def create_bathymetry_grid(ds_subset, min_lon, max_lon, min_lat, max_lat, cell_size_m=500, vessel_height=0):
    ...
    for r in range(num_lat_cells):
        for c in range(num_lon_cells):
            ...
            # Debugging: Print elevation and vessel_height for a few cells
            # if r % 50 == 0 and c % 50 == 0:
            #     print(f"Cell ({r}, {c}): Lat={cell_lat:.2f}, Lon={cell_lon:.2f}, Elevation={elevation:.2f}, Vessel_height_underwater={vessel_height:.2f}")

            if elevation >= -vessel_height:
                grid[r, c] = 1 # Obstacle (too shallow or land)
            else:
                grid[r, c] = 0 # Walkable (deep enough)
...
def lat_lon_to_grid_coords(lat, lon, min_lat, min_lon, lat_step, lon_step, num_lat_cells, num_lon_cells):
    ...
    # print(f"  Lat/Lon ({lat:.4f}, {lon:.4f}) mapped to grid ({row}, {col})") # Debug print
    return row, col
...
if __name__ == "__main__":
    ...
    # 1. Get optimal path route from main.py logic
    print("Generating optimal path route...")
    optimal_path_route, G_main, selected_ports_main = get_optimal_path_route(ports_gdf, vessel)

    if optimal_path_route:
        print("\nOptimal path route:", optimal_path_route)
        
        # Load bathymetry data for A*
        ...
        cell_size_m_astar = 5000 # 1km x 1km grid cells

        all_astar_paths_latlon = []
        
        # Prepare plot for all A* paths
        ...
        # Create the bathymetry grid once for the entire region
        ...
        # 2. For each segment, generate A* route
        for i in range(len(optimal_path_route) - 1):
            ...
            # Adjust start/end grid points if they are obstacles
            # Increased search radius to 10 cells
            search_radius = 10
        ...
        plt.ylabel("Latitude")
        # Create a single legend entry for A* paths if multiple segments are plotted
