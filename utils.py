...
vessel = {
    ...
    "size": (300, 100, 5),  
    "percent_of_height_underwater": 0.0,
}
...
def generate_multi_leg_astar_path_and_landmarks(vessel_data, sequenced_destination_points_latlon, 
                                                ...
                                                search_range_km=2): # Added search_range_km
    """
    ...
    print(f"Vessel underwater: {vessel_height_underwater_calc}")
...
def create_bathymetry_grid(ds_subset, min_lon, max_lon, min_lat, max_lat, cell_size_m=500, vessel_height=0):
    ...
    # Calculate initial steps based on cell_size_m
    lat_step_initial = cell_size_m * lat_deg_per_m
    lon_step_initial = cell_size_m * lon_deg_per_m

    # Calculate number of cells based on initial steps
    num_lat_cells = int(np.ceil((max_lat - min_lat) / lat_step_initial))
    num_lon_cells = int(np.ceil((max_lon - min_lon) / lon_step_initial))

    grid = np.zeros((num_lat_cells, num_lon_cells), dtype=int)
    elevation_data = np.zeros((num_lat_cells, num_lon_cells), dtype=float)
    
    # Generate grid coordinates using linspace
    grid_lats = np.linspace(min_lat, max_lat, num_lat_cells)
    grid_lons = np.linspace(min_lon, max_lon, num_lon_cells)

    # Recalculate lat_step and lon_step based on the actual grid generated by linspace
    # This ensures consistency with the elevation_data's coordinates and grid_params
    if num_lat_cells > 1:
        lat_step = (grid_lats[-1] - grid_lats[0]) / (num_lat_cells - 1)
    else:
        lat_step = lat_step_initial # Fallback if only one cell

    if num_lon_cells > 1:
        lon_step = (grid_lons[-1] - grid_lons[0]) / (num_lon_cells - 1)
    else:
        lon_step = lon_step_initial # Fallback if only one cell
